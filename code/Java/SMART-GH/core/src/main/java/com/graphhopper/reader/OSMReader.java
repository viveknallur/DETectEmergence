...
public class OSMReader implements DataReader
{
    ...
    public TIntLongMap edgeIdToOsmIdMap;
    ...
    public HashMap<Integer, Long> returnConvertedOSM () {
    	HashMap<Integer, Long> returnMap = new HashMap<Integer, Long>();
    	int[] keyInts = edgeIdToOsmIdMap.keys();
    	for(int i = 0; i < keyInts.length; i++){
    		returnMap.put(keyInts[i], edgeIdToOsmIdMap.get(keyInts[i]));
    	}
    	return returnMap;
    }
    ...
    EdgeIteratorState addEdge( int fromIndex, int toIndex, PointList pointList, long flags, long wayOsmId )
    {
        ...
        EdgeIteratorState iter = graphStorage.edge(fromIndex, toIndex).setWayId(wayOsmId).setDistance(towerNodeDistance).setFlags(flags);
        //Eamonn
        //Eamonn
        //System.out.println(iter.getEdge()+ " , " + wayOsmId);
        nodeTranslator.put(iter.getEdge(), wayOsmId);
        if (nodes > 2)
        {
            if (doSimplify)
