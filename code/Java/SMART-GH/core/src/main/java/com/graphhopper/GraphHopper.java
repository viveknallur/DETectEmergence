...
public class GraphHopper implements GraphHopperAPI
{
    ...
    public HashMap <Integer,Long> nodeTranslator;
    ...
    public GraphHopper init( CmdArgs args )
    {
       ...
       /* System.out.println("Init GH");
        System.out.println("graph.location= " + args.get("graph.location", ""));
		System.out.println("config= " + args.get("config", ""));
		System.out.println("osm.reader= "+ args.get("osm.reader", ""));
		System.out.println("osmreader.osm= "+ args.get("osmreader.osm", ""));*/
        ...
        nodeTranslator = new HashMap<Integer, Long>();
    ...
    // Eamonn
    public void writeMyMap() throws Exception{
        String folder = System.getProperty("InputFolder");   
    	//FileOutputStream fos = new FileOutputStream("//home/eamonn/workspace/SMART-GH/maps/NewYork-gh/NewYorkOsm.ser");
        FileOutputStream fos = new FileOutputStream(folder+"/GH/maps/NewYorkOsm.ser");
           ObjectOutputStream oos = new ObjectOutputStream(fos);
           oos.writeObject(nodeTranslator);
           oos.close();
    }
    
    public void readMyMap() throws Exception{
    	 //FileInputStream fis = new FileInputStream("//home/eamonn/workspace/SMART-GH/maps/NewYork-gh/NewYorkOsm.ser");
    	String folder = System.getProperty("InputFolder");   
    	FileInputStream fis = new FileInputStream(folder+"/GH/maps/NewYorkOsm.ser");
         ObjectInputStream ois = new ObjectInputStream(fis);
         nodeTranslator = (HashMap<Integer, Long>) ois.readObject();
         ois.close();
         if (nodeTranslator == null){
         	System.out.println("Could not obtain OSM IDs");
         }else {
         	/*try{
         	 FileWriter fstream;
             BufferedWriter out;
             ...
             fstream = new FileWriter("//home/eamonn/values2.txt");
             out = new BufferedWriter(fstream);
             
             int headers[] = new int[nodeTranslator.keySet().size()];  
             int i = -1;  
             for (Iterator<Integer> iterator = nodeTranslator.keySet().iterator(); iterator.hasNext(); )
             	headers[++i] = iterator.next();  
             
             for(i = 0; i < headers.length; i++)
            	 out.write(headers[i] + " , " + nodeTranslator.get(headers[i]) + "\n");
    ...
    protected OSMReader initOSMReader( OSMReader reader )
    {
        ...
        OSMReader r = reader.setOSMFile(osmTmpFile).
                setElevationProvider(eleProvider).
                setWorkerThreads(workerThreads).
                setEncodingManager(encodingManager).
                setWayPointMaxDistance(wayPointMaxDistance);
        //Eamonn TO Get the OSM IDs
        nodeTranslator = r.nodeTranslator;
        //nodeTranslator = r.returnConvertedOSM();
        return r;
    }
    ...
    public GHResponse route( GHRequest request )
    {
        ...
        //Eamonn
        ArrayList<Integer> edgeIds = response.getEdgeIds();
        Iterator<Integer> it = edgeIds.iterator();
        ArrayList<Long> osmIds = new ArrayList<Long>();
        osmIds.add(response.getStartOsm());
        while(it.hasNext()){
        	int edge = it.next();
        	if(nodeTranslator.get(edge) != null){
        		long osm = nodeTranslator.get(edge);
        		if(osmIds.size() > 0){
		        	if(osmIds.get(osmIds.size() - 1) != osm){
		        		osmIds.add(nodeTranslator.get(edge));
		        	}
        		}else{
        			osmIds.add(nodeTranslator.get(edge));
        		}
        ...
        osmIds.add(response.getEndOsm());
        response.setOsmIds(osmIds);
        //Eamonn
        return response;
    }
    ...
    protected List<Path> getPaths( GHRequest request, GHResponse rsp )
    {
        ...
        for (int placeIndex = 1; placeIndex < points.size(); placeIndex++)
        {
            ...
            //Eamonn
            rsp.setStartOsm(nodeTranslator.get(fromRes.getClosestEdge().getEdge()));
            rsp.setEndOsm(nodeTranslator.get(toRes.getClosestEdge().getEdge()));
            // Eamonn FROM THIS CAN GET START EDGE ID
            //if (path.getMillis() < 0)
