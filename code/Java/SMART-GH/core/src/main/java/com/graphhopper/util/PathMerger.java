...
public class PathMerger
{
    ...
    public void doWork( GHResponse rsp, List<Path> paths, Translation tr )
    {
        ...
        for (int pathIndex = 0; pathIndex < paths.size(); pathIndex++)
        {
            ...
            //Eamonn
            List<EdgeIteratorState> edgeList = path.calcEdges();
            Iterator<EdgeIteratorState> it = edgeList.iterator();
            edgeIds = new ArrayList<Integer>();
            ArrayList<String> edgeName = new ArrayList<String>();
            while(it.hasNext()){
            	EdgeIteratorState thisEdge = it.next();
            	edgeIds.add(thisEdge.getEdge());
            	edgeName.add(thisEdge.getName());
            	//System.out.println("Hey : " + thisEdge.getEdge());
            }
